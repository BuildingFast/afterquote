...
import ListComments from "./OrderComments";
...
const OrderCommentField = dynamic(
  () => import("~/components/OrderCommentField"),
  {
    ssr: false,
    loading: () => (
...
const OrderCommentActionModal = dynamic(
  () => import("~/components/OrderCommentActionModal"),
  {
    ssr: false,
  }
);
...
const OrderComment: React.FC<CommentProps> = ({
  comment,
  compact = false,
  identifiable = false,
  variant = "primary",
    ...
    >
      ...
      >
          ...
          <ShouldRender if={linkToPost && !loading}>
            ...
            >
              ...
              <Link
                ...
                href={`/posts/${comment?.salesOrderId}?highlightedComment=${comment?.id}`}
                as={`/posts/${comment?.salesOrderId}`}
              >
                {/*comment?.rfsq?.companyName*/}
              </Link>
            </p>
          </ShouldRender>
        ...
        {replying && !shouldRenderModals && (
          <OrderCommentField parentId={comment?.id} onCommented={onCommented} />
        )}
      </div>
      ...
      <ShouldRender if={shouldRenderModals}>
        <OrderCommentActionModal
          parentComment={comment}
          openState={replyState}
        />
      </ShouldRender>
    </div>
  );
