...
const PublicShareOrder: NextPage = () => {
  const [viewerUrl, setViewerUrl] = useState("");
  const [, setViewerFileId] = useState("");
  const router = useRouter();
  const { uid } = router.query;
  const salesOrderUid = String(uid);
  const salesOrder = api.salesOrder?.getOnePublic.useQuery(salesOrderUid, {
    cacheTime: 100,
  });
  ...
  const orderCustomFieldSchema =
    api.organization?.getOrganizationOrderCustomFieldSchemaPublic.useQuery(
      salesOrderUid
    );
  const salesOrderFilesS3Urls =
    api.orderFiles.getS3UrlsForCustomerPublicLink.useQuery({
  ...
  return (
    <>
      <Head>
        <title>Customer Order Details • Afterquote</title>
      </Head>
...
export default PublicShareOrder;
