...
const OrderCommentField: React.FC<Props> = ({ parentId, onCommented }) => {
  ...
  const { mutate, error: createCommentError } =
    api.comments.createCommentForSalesOrder.useMutation({
      onSuccess: () => {
        ...
        utils.comments.getCommentsForSalesOrder.invalidate();
      },
    });

  const createNotification =
    api.notification.createNotificationForSalesOrderChat.useMutation();
  ...
  function sendNotificationToUser(id: string | undefined) {
    if (id) {
      createNotification.mutate({
        message: "Tagged in Chat Message on Sales Order",
        notificationFor: id,
  ...
  return (
    <form className="mt-1" onSubmit={(e) => onSubmit(e)}>
      ...
      <div className="w-full sm:flex sm:justify-between">
        <Button className="mt-2" variant={"outline"} size="sm" type="submit">
...
export default OrderCommentField;
